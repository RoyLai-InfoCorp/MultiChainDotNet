...
namespace MultiChainDotNet.Core.MultiChainAsset
{
	public class MultiChainAssetCommand : MultiChainCommandBase
	{
		...
		public async Task<MultiChainResult<string>> SendFromAsync(string addressFrom, string addressTo, double amt, string assetName = null, object data = null)
		{
			if (assetName is null)
			{
				if (data is null)
					return await JsonRpcRequestAsync<string>("sendfrom", addressFrom, addressTo, amt);
				return await JsonRpcRequestAsync<string>("sendfrom", addressFrom, addressTo, new Dictionary<string, object> { { "", amt }, { "data", new { json = data } } });
			}
			if (data is null)
				return await JsonRpcRequestAsync<string>("sendfrom", addressFrom, addressTo, new Dictionary<string, object> { { assetName, amt } });
			return await JsonRpcRequestAsync<string>("sendfrom", addressFrom, addressTo, new Dictionary<string, object> { { assetName, amt }, { "data", new { json = data } } });
		}
